...
import { DATA_DIR, getBackend } from "./storage.js";
...
async function readSupabaseOverrides() {
  ...
  const { data, error } = await client
    .from("aivio_cms_config")
    .select("overrides")
    .eq("id", "robot_overrides")
    .maybeSingle();
  if (error) {
    console.error("CMS READ ERROR (SUPABASE):", error.message);
    return null;
  }
  return data?.overrides && typeof data.overrides === "object" ? data.overrides : {};
}

async function writeSupabaseOverrides(overrides) {
  const client = await getSupabaseClient();
  if (!client) return;
  const { error } = await client.from("aivio_cms_config").upsert({
    id: "robot_overrides",
    overrides,
    updated_at: new Date().toISOString()
  });
  if (error) console.error("CMS WRITE ERROR (SUPABASE):", error.message);
}
...
export async function readCmsOverrides() {
  ...
  if (backend.type === "supabase") {
    ...
    return remote && Object.keys(remote).length > 0 ? remote : localOverrides;
  }
...
export async function writeCmsOverrides(overrides) {
  ...
  if (backend.type === "supabase") {
    await writeSupabaseOverrides(overrides);
    return;
  }
