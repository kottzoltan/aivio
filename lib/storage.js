...
import { getSupabaseClient, getSupabaseLastError, shouldUseSupabase } from "./supabase.js";
...
export async function getStorageInfo() {
  ...
  return {
    ...
    supabasePreferred: shouldUseSupabase(),
    supabaseReady: backend.type === "supabase",
    supabaseLastError: getSupabaseLastError(),
...
async function supabaseWriteRecord(client, collection, docId, data) {
  const now = new Date().toISOString();
  const createdAt = data.createdAt || now;
  const payload = { ...data, id: String(docId), createdAt, updatedAt: now };

  const { error } = await client.from("aivio_records").upsert({
    collection,
    id: String(docId),
    data: payload,
...
export async function writeObject(collection, docId, data) {
  ...
  if (backend.type === "supabase") {
    return supabaseWriteRecord(backend.client, collection, docId, {
      ...data,
...
export async function appendRecord(collection, item) {
  ...
  if (backend.type === "supabase") {
    ...
    await supabaseWriteRecord(backend.client, collection, record.id, { ...record, createdAt });
    return record;
  }
